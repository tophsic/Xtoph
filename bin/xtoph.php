...
class Xtoph
{
    ...
    protected function _tryClientLoad()
    {
        ...
        $fh = @fopen('Xtoph/Tool/Framework/Client/Console.php', 'r', true);
        if (!$fh) {
            return $this->_clientLoaded; // false
        } else {
            fclose($fh);
            unset($fh);
            include 'Xtoph/Tool/Framework/Client/Console.php';
...
***************************** Xtoph ERROR ********************************
In order to run the xtoph command, you need to ensure that Zend Framework
and Xtoph Framework are inside your include_path.  There are a variety of ways
that you can ensure that this xtoph command line tool knows where the Zend
Framework and Xtoph Framework library is on your system, but not all of them can
be described here.

The easiest way to get the xtoph command running is to give it the include
path via an environment variable ZEND_TOOL_INCLUDE_PATH or
ZEND_TOOL_INCLUDE_PATH_PREPEND with the proper include path to use,
then run the command "xtoph --setup".  This command is designed to create
a storage location for your user, as well as create the zf.ini file
that the xtoph command will consult in order to run properly on your
system.

Example you would run:

$ ZEND_TOOL_INCLUDE_PATH=/path/to/library xtoph --setup
...
EOS;
    ...
    protected function _runTool()
    {
        ...
        $configOptions['classesToLoad'] = 'Xtoph_Tool_Project_Provider_Manifest';

        $console = new Xtoph_Tool_Framework_Client_Console($configOptions);
        $console->dispatch();
    }
